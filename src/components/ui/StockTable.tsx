...
interface Stock {
  ...
  interface StockTableProps {
    stocks: Stock[];
    onTransaction: (transaction: { stock: Stock; quantity: number }) => void; // Callback function to handle the value
    wallet: string
  }

  const StockTable: React.FC<StockTableProps> = ({ stocks, onTransaction, wallet }) => {
    ...
    const isInWallet = (symbol:string) => {
              ...
              {stocks.map((stock) => (
                <TableRow key={stock.symbol}>
                  ...
                  <TableCell className="text-right">
                    ${selectedQuantities[stock.symbol] > 0 
                      ? (stock.price * selectedQuantities[stock.symbol]).toFixed(2) 
